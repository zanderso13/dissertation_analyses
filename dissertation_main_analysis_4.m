...
% end. 
basedir = '/Users/zacharyanderson/Documents/ACNlab/BrainMAPD/mid_corr_matrices/final_corr_mats';
cd(basedir)


fnames = filenames('*.mat');
    
load(fullfile('/Users/zacharyanderson/Documents/ACNlab/BrainMAPD/beta_series/final_motion_exclusion.mat'));
...
if whole_brain_networks == 1
    ...
    for f = 1:length(fnames)
        load(fnames{f});
        pid=fnames{f}(1:5);
        final_brain_vs=fmri_data('/Users/zacharyanderson/Documents/ACNlab/BrainMAPD/beta_series/sub-10001/ses-2/gain_contrasts/run-1/beta_0001.nii');
        final_brain_vs.dat = atanh(final_corr_vs_whole);
        final_brain_vs.fullpath = strcat(pid,'_antgain.nii');
        write(final_brain_vs)
    end
    ...
    final_brain_vs = fmri_data(full_fnames);
    final_brain_vs.dat(isnan(final_brain_vs.dat)) = 0;
    
    %% Aim 1: loading in data for basic brain analyses. 
    final_brain_vs.X = [regressors.longGeneralDistress,regressors.sex,regressors.site,regressors.meds,regressors.race,regressors.ethnicity,regressors.inc];
    
    % remove NaNs related to inflammation
    final_brain_vs.dat(:, isnan(regressors.inflammation(:,1))) = [];
    full_fnames(isnan(regressors.inflammation(:,1)),:)=[];
    fnames_mat(isnan(regressors.inflammation(:,1)),:)=[];
    final_brain_vs.X(isnan(regressors.inflammation(:,1)),:) = [];
    regressors(isnan(regressors.inflammation(:,1)),:) = [];
    
    % remove NaNs related to family income
    final_brain_vs.dat(:, isnan(regressors.inc(:,1))) = [];
    full_fnames(isnan(regressors.inc(:,1)),:)=[];
    fnames_mat(isnan(regressors.inc(:,1)),:)=[];
    final_brain_vs.X(isnan(regressors.inc(:,1)),:) = [];
    regressors(isnan(regressors.inc(:,1)),:) = [];
        
    
    % remove NaNs related to symptoms
    final_brain_vs.dat(:, isnan(regressors.longGeneralDistress(:,1))) = [];
    full_fnames(isnan(regressors.longGeneralDistress(:,1)),:)=[];
    fnames_mat(isnan(regressors.longGeneralDistress(:,1)),:)=[];
    final_brain_vs.X(isnan(regressors.longGeneralDistress(:,1)),:) = [];
    regressors(isnan(regressors.longGeneralDistress(:,1)),:) = [];
    
    statobj = regress(final_brain_vs);
...
if whole_brain_mediation_analysis == 1
    final_brain_vs.fullpath = '/Users/zacharyanderson/Documents/ACNlab/BrainMAPD/beta_series/final_matrices/anticipation_combined_runs_with_hyp/mediation_results_antgain/antgain_brain.nii';
    mkdir mediation_results_antgain
    cd mediation_results_antgain
    mkdir general_distress
    mkdir anhedonia
    mkdir fears
    write(final_brain_vs,'overwrite')
    ...
    resultsgendis = mediation_brain(regressors.inflammation, regressors.longGeneralDistress, final_brain_vs.fullpath,'names',names,'mask', mask,'noverbose');    
    ...
    resultsanhed = mediation_brain(regressors.inflammation, regressors.longAnhedonia, final_brain_vs.fullpath,'names',names,'mask', mask,'noverbose');
    ...
    resultsfears = mediation_brain(regressors.inflammation, regressors.longFears, final_brain_vs.fullpath,'names',names,'mask', mask,'noverbose');
